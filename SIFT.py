...
def timeit(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print('{} end, cost {}s'.format(func_name, end - start))
        return retval
    return wrapper
...
@timeit
def getDoG_new1(img, n, sigma0, S=None, O=None):
    '''
    ...
    for i in range(O):
        ...
        for j in range(S):
            if i == 0 and j == 0:
                ...
                if dim % 2 == 0:
                    dim += 1
                blur_img = cv2.GaussianBlur(img, ksize=(dim, dim), \
                    sigmaX=sig, sigmaY=sig) 
                GuassianPyramid[-1].append(blur_img)
            elif j == 0:
                base_img = GuassianPyramid[-2][-3]
                GuassianPyramid[-1].append(downsample(base_img))
            else:
                sig = math.sqrt(sigma[i][j]**2 - sigma[i][0]**2)
                dim = int(6*sig + 1)
                if dim % 2 == 0:
                    dim += 1
                blur_img = cv2.GaussianBlur(GuassianPyramid[i][0], ksize=(dim, dim), \
                    sigmaX=sig, sigmaY=sig)
                GuassianPyramid[-1].append(blur_img)
            # GuassianPyramid[-1].append(convolve(GuassianKernel(sigma[i][j], dim),samplePyramid[i],[dim//2,dim//2,dim//2,dim//2],[1,1]))
            # GuassianPyramid[-1].append(cv2.GaussianBlur(samplePyramid[i], ksize=(dim, dim),\
                #  sigmaX=sigma[i][j], sigmaY=sigma[i][j]))
...
@timeit
def getDoG_new2(img, n, sigma0, S=None, O=None):
    '''
    :param img: the original img.
    :param sigma0: sigma of the first stack of the first octave. default 1.52 for complicate reasons.
    :param n: how many stacks of feature that you wanna extract.
    ...
    for i in range(O):
        ...
        for j in range(S):
            ...
            if i == 0 and j == 0:
                ...
                sig = math.sqrt(sigma[i][j]**2 - sigma[i][j-1]**2)
                dim = int(6*sig + 1)
                if dim % 2 == 0:
                    dim += 1
                blur_img = cv2.GaussianBlur(GuassianPyramid[-1][-1], ksize=(dim, dim), \
                    sigmaX=sig, sigmaY=sig)
                GuassianPyramid[-1].append(blur_img)
            # GuassianPyramid[-1].append(convolve(GuassianKernel(sigma[i][j], dim),samplePyramid[i],[dim//2,dim//2,dim//2,dim//2],[1,1]))
            # GuassianPyramid[-1].append(cv2.GaussianBlur(samplePyramid[i], ksize=(dim, dim),\
...
@timeit
def getDoG_opencv(img, n, sigma0, S=None, O=None):
    '''
    opencv version
    :param img: the original img.
    :param sigma0: sigma of the first stack of the first octave. default 1.52 for complicate reasons.
    ...
    sigs_ = sigma[0]
    for i in range(1,S):
        r_sig = math.sqrt(sigs_[i]**2 - sigs_[i - 1]**2)
        sig_.append(r_sig)
...
def SIFT(img, saveDoGimgs=False):
    SIFT_SIGMA = 1.6
    ...
    if saveDoGimgs:
        for ith, i in enumerate(DoG):
            for jth, j in enumerate(i):
                cv2.imwrite(f'results/DoG_{ith}_{jth}.jpg', j.astype(np.uint8))
...
def drawLines(X1,X2,Y1,Y2,dis,img,num =15):
    ...
    for i in range(info.shape[0]):
        ...
        cv2.line(img,(int(info[i,0]),int(info[i,2])),(int(info[i,1]),int(info[i,3])),rand_color,3)
    if len(img.shape) == 2:
...
if __name__ == '__main__':
    ...
    img_copy = img.copy()
    for i in keyPoints:
        cv2.circle(img_copy, (int(i[0]), int(i[1])), 2, (0, 0, 255), -1)
    cv2.imwrite(f'results/KeyPoints.jpg', img_copy)


    origimg2 = plt.imread('2_2.jpg')
    if len(origimg.shape) == 3:
